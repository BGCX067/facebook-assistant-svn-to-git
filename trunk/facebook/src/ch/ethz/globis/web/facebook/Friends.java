		...
		try {
			...
			final URL url = new URL(
					"https://api.facebook.com/method/fql.query?access_token="
							+ accessToken
							+ "&query="
							+ URLEncoder.encode("SELECT name,uid,pic_square FROM user WHERE uid IN ( SELECT uid2 FROM friend WHERE uid1=me())",
							"UTF-8"));
//									.encode("SELECT name,uid,pic_square FROM user WHERE uid IN ( SELECT uid2 FROM friend WHERE uid1=me() ) LIMIT 40 ",
//											"UTF-8"));
